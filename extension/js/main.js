...
(() => {
  ...
  if (PP.native) {
    ...
    // Plain browser tab: either the Chrome extension, or this served up directly for development. Either way,
    // drag & drop a PDF, ZIP or images anywhere.
    if (window.chrome && chrome.runtime && chrome.runtime.getManifest) {
      try { $('#app-version').textContent = `Version ${chrome.runtime.getManifest().version}`; } catch (_) { /* not actually the extension */ }
    }
