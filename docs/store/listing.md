...
## Tried and abandoned: a night-mode button floating on top of Edge's own PDF viewer

Not possible. Browsers don't allow one extension to run any code inside another extension's pages, and Edge's
built-in PDF viewer is implemented as exactly that (a separate, internal extension) — confirmed directly by
Edge's own manifest validator rejecting a content script matching its origin. The only way around it would be
redirecting every PDF you open into Pocket PDF's own viewer instead of Edge's, which trades away Edge's built-in
PDF tools (highlighter, Ask Copilot, translate, read-aloud) on every PDF — not done, since it wasn't worth that
cost. Night mode still works fully inside the app itself.
